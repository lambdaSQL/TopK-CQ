...
public:
    ...
    unordered_map<long, vector<std::pair<long, long>>> x_to_y_with_score;
    unordered_map<int, vector<int>> y_to_x;
    unordered_map<long, vector<std::pair<long, long>>> y_to_x_with_score;
    std::ofstream null_stream;

    int space_used_bytes;
    set<long long int> checkpoints { 8, 64, 256, 1024, 4096, 16384, 32768, 1024000, 2048000};//, 10000, 100000, 1000000, 10000000, 100000000, 200000000 , 300000000,
...
//    }
    ...
    void load(std::string dataset) {
        ...
        std::cout << "starting load of dataset: " << dataset << std::endl;
        std::ifstream file(BASE + dataset + ".csv");
        cout << BASE+dataset+".csv" << endl;
        std::string   line;
        unordered_map<long, std::set<std::pair<long, long>>> temp_map;
        while(std::getline(file, line)) {
            std::stringstream  lineStream(line);
            long x, y; char ch1, ch2; unsigned long w;
            while(lineStream >> x >> ch1 >> y >> ch2 >> w) {
                std::pair<int, int> temp_pair = std::make_pair(y, w);
                temp_map[x].insert(temp_pair);
            }
        }
        for (auto& it : temp_map) {
            std::vector<pair<long,long>> v;
            v.assign(it.second.begin(), it.second.end());
            x_to_y_with_score[it.first] = v;
            std::sort(x_to_y_with_score[it.first].begin(), x_to_y_with_score[it.first].end());
        }
        for (auto&it : x_to_y_with_score) {
            for (auto&val : it.second) {
                std::pair<long,long> temp_pair = std::make_pair(it.first, val.second);
                y_to_x_with_score[val.first].push_back(temp_pair);
            }
        }
        for(auto& it : y_to_x_with_score) {
            std::sort(it.second.begin(), it.second.end());
        }
        std::cout << "completed load of dataset: " << dataset << std::endl;
    }
...
class PATH_TWO_FULL : public Algorithm {
public:
    ...
    virtual void execute(std::string dataset, int limit) {
        ...
        for (auto &it: x_to_y_with_score) {
            for (auto &val: it.second) {
                if (tot_size <= it.first) tot_size = it.first;
                if (tot_size <= val.second) tot_size = val.second;
            }
        }

        priority_queue<tuple<unsigned long, long, long, long>,
                std::vector<tuple<unsigned long, long, long, long>>,
                less<tuple<unsigned long, long, long, long>>> pq1;
        std::vector<priority_queue<tuple<unsigned long, long, long>,
                std::vector<tuple<unsigned long, long, long>>,
                less<tuple<unsigned long, long, long>>>> pq2(tot_size+100);
        ...
        for (auto &it: x_to_y_with_score) {
            for (auto &itd: it.second) {
                long it1 = it.first;
                long it2 = itd.first;
                long weight = itd.second;
                auto t = make_tuple(weight, it1, it2);
                pq2[it1].push(t);
            }
        }
        cout << "HERE" << endl;
        for (auto &it: x_to_y_with_score) {
            for (auto &itd: it.second) {
                long it1 = it.first;
                long it2 = itd.first;
                unsigned long weight = itd.second;
                if (!pq2[it2].empty()) {
                    auto t1 = pq2[it2].top();
                    auto w1 = std::get<0>(t1);
                    weight = itd.second + w1;
                    auto t = make_tuple(weight, it1, it2, 0);
                    pq1.push(t);
                }
            }
        }
        ...
        while (!pq1.empty()) {
            const auto t = pq1.top();
            const auto x = std::get<1>(t);
            const auto y = std::get<2>(t);
            const auto w = std::get<0>(t);
            const auto p1 = std::get<3>(t);

            pq1.pop();
            vector<tuple<unsigned long, long, long>> temp2(p1);
            for (long i = 0; i< p1; i++) {
                const auto tuple = pq2[y].top();
                temp2[i] = tuple;
                pq2[y].pop();
            }
            const auto t2 = pq2[y].top();
            const auto z = std::get<2>(t2);
            const auto w2 = std::get<0>(t2);
            pq2[y].pop();

            if (!pq2[y].empty()) {
                auto new_t2 = make_tuple(std::get<0>(pq2[y].top())+w-w2, x, y, p1+1);
                //cout << std::get<0>(new_t2) << " " << std::get<1>(new_t2) << " " << std::get<2>(new_t2) << " " << std::get<3>(new_t2) << " " << std::get<4>(new_t2)  << endl;
                pq1.push(new_t2);
            }

            pq2[y].push(t2);
            //cout << get<0>(t2) << " " << get<1>(t2) << " " << get<2>(t2) << endl;
            //cout << get<0>(pq2[y].top()) << " " << get<1>(pq2[y].top()) << " " << get<2>(pq2[y].top()) << endl;
            for (long i =0; i < p1; i++) {
                pq2[y].push(temp2[i]);
            }
            null_stream << x << " " << y << " " << z << " " << w << " " << w2 <<  endl;//" " << p1 << " " << p2 << endl;

            counter += 1;

            if (checkpoints.find(counter) != checkpoints.end()) {
                gettimeofday(&temp, NULL);
                long long int timediff = (temp.tv_sec - starting.tv_sec)*1e6 + (temp.tv_usec - starting.tv_usec);
                pair<long long int, long long int> p = make_pair(timediff, pq1.size());
                time_space[counter] = p;
                cout << counter << " tuples in time and space " << timediff << " " << pq1.size() << endl;
            }
        ...
        cout << "number of tuples enumerated : " << counter << endl;
        gettimeofday(&ending, NULL);
        cout << "All: " << (ending.tv_sec - starting.tv_sec)*1e6 + (ending.tv_usec - starting.tv_usec) << endl;
        cout << "end executing " << name() << endl;
    }

};

class PATH_THREE_FULL : public Algorithm {
public:
    ...
    virtual void execute(std::string dataset, int limit) {
        ...
        priority_queue<tuple<unsigned long, long, long, long, long>,
                std::vector<tuple<unsigned long, long, long, long, long>>,
                less<tuple<unsigned long, long, long, long, long>>> pq1;
        std::vector<priority_queue<tuple<unsigned long, long, long>,
                std::vector<tuple<unsigned long, long, long>>,
                less<tuple<unsigned long, long, long>>>> pq2(tot_size+100);
        std::vector<priority_queue<tuple<unsigned long, long, long>,
                std::vector<tuple<unsigned long, long, long>>,
                less<tuple<unsigned long, long, long>>>> pq3(tot_size+100);
        ...
        for (auto &it: x_to_y_with_score) {
            for (auto &itd: it.second) {
                ...
                if (!pq2[it2].empty()) {
                    ...
                    auto w1 = std::get<0>(t1);
                    weight = itd.second + w1;
                    auto t = make_tuple(weight, it1, it2, 0, 0);
                    pq1.push(t);
                }
            }
        }
        ...
        while (!pq1.empty()) {
            ...
            const auto w = std::get<0>(t);
            const auto p1 = std::get<3>(t);
            const auto p2 = std::get<4>(t);
            pq1.pop();
            vector<tuple<unsigned long, long, long>> temp2(p1);
            vector<tuple<unsigned long, long, long>> temp3(p2);
            for (long i = 0; i< p1; i++) {
                const auto tuple = pq2[y].top();
                temp2[i] = tuple;
                pq2[y].pop();
            ...
            for (long i = 0; i< p2; i++) {
                const auto tuple = pq3[z].top();
                //cout << get<0>(tuple) << " " << get<1>(tuple) << " " << get<2>(tuple) << endl;
                temp3[i] = tuple;
                pq3[z].pop();
            }
            const auto t3 = pq3[z].top();
            const auto a = std::get<2>(t3);
            const auto w3 = std::get<0>(t3);
            pq3[z].pop();

            if (!pq3[z].empty()) {
                auto new_t1 = make_tuple(std::get<0>(pq3[z].top())+w-w3, x, y, p1, p2+1);
                //cout << std::get<0>(new_t1) << " " << std::get<1>(new_t1) << " " << std::get<2>(new_t1) << " " << std::get<3>(new_t1) << " " << std::get<4>(new_t1)  << endl;
                pq1.push(new_t1);
            }
            pq3[z].push(t3);
            for (long i = 0; i < p2; i++) {
            ...
            if (!pq2[y].empty() && p2 == 0) {
                auto new_t2 = make_tuple(std::get<0>(pq2[y].top())+w-w2, x, y, p1+1, 0);
                //cout << std::get<0>(new_t2) << " " << std::get<1>(new_t2) << " " << std::get<2>(new_t2) << " " << std::get<3>(new_t2) << " " << std::get<4>(new_t2)  << endl;
                pq1.push(new_t2);
            }
            ...
            null_stream << x << " " << y << " " << z << " " << a << " " << w << " " << w2 << " " << w3 << endl;//" " << p1 << " " << p2 << endl;
...
class PATH_FOUR_FULL : public Algorithm {
public:
    ...
    virtual void execute(std::string dataset, int limit) {
        ...
        gettimeofday(&starting, NULL);
        load(dataset);
        //std::vector<long> new_candidate_count_vec;
        long counter = 1;
        unordered_map<long long int, pair<long long int, long long int>> time_space;
        long tot_size = 0;
        for (auto &it: x_to_y_with_score) {
        ...
        priority_queue<tuple<unsigned long, long, long, long, long, long>,
                std::vector<tuple<unsigned long, long, long, long, long, long>>,
                less<tuple<unsigned long, long, long, long, long, long>>> pq1;
        std::vector<priority_queue<tuple<unsigned long, long, long>,
                std::vector<tuple<unsigned long, long, long>>,
                less<tuple<unsigned long, long, long>>>> pq2(tot_size+100);
        std::vector<priority_queue<tuple<unsigned long, long, long>,
        ...
        std::vector<priority_queue<tuple<unsigned long, long, long>,
                std::vector<tuple<unsigned long, long, long>>,
                less<tuple<unsigned long, long, long>>>> pq4(tot_size+100);
        cout << "HERE" << endl;
        //long vec_counter = 0;
        for (auto &it: x_to_y_with_score) {
            for (auto &itd: it.second) {
        ...
        for (auto &it: x_to_y_with_score) {
            for (auto &itd: it.second) {
                ...
                if (!pq2[it2].empty()) {
                    ...
                    auto w1 = std::get<0>(t1);
                    weight = itd.second + w1;
                    auto t = make_tuple(weight, it1, it2, 0, 0, 0);
                    pq1.push(t);
                }
        ...
        while (!pq1.empty()) {
            ...
            const auto p1 = std::get<3>(t);
            const auto p2 = std::get<4>(t);
            const auto p3 = std::get<5>(t);
            pq1.pop();
            vector<tuple<unsigned long, long, long>> temp2(p1);
            vector<tuple<unsigned long, long, long>> temp3(p2);
            vector<tuple<unsigned long, long, long>> temp4(p3);
            for (long i = 0; i< p1; i++) {
                const auto tuple = pq2[y].top();
                temp2[i] = tuple;
                pq2[y].pop();
            ...
            for (long i = 0; i< p3; i++) {
                const auto tuple = pq4[a].top();
                //cout << get<0>(tuple) << " " << get<1>(tuple) << " " << get<2>(tuple) << endl;
                temp4[i] = tuple;
                pq4[a].pop();
            }
            const auto t4 = pq4[a].top();
            const auto b = std::get<2>(t4);
            const auto w4 = std::get<0>(t4);
            pq4[a].pop();

            if (!pq4[a].empty()) {
                auto new_t1 = make_tuple(std::get<0>(pq4[a].top())+w-w4, x, y, p1, p2, p3+1);
                //cout << std::get<0>(new_t1) << " " << std::get<1>(new_t1) << " " << std::get<2>(new_t1) << " " << std::get<3>(new_t1) << " " << std::get<4>(new_t1)  << endl;
                pq1.push(new_t1);
            }
            pq4[a].push(t4);
            ...
            if (!pq3[z].empty() && p3 == 0) {
                auto new_t3 = make_tuple(std::get<0>(pq3[z].top())+w-w3, x, y, p1, p2+1, 0);
                //cout << std::get<0>(new_t2) << " " << std::get<1>(new_t2) << " " << std::get<2>(new_t2) << " " << std::get<3>(new_t2) << " " << std::get<4>(new_t2)  << endl;
                pq1.push(new_t3);
            }
            ...
            if (!pq2[y].empty() && p2 == 0 && p3 == 0) {
                auto new_t2 = make_tuple(std::get<0>(pq2[y].top())+w-w2, x, y, p1+1, 0, 0);
                //cout << std::get<0>(new_t2) << " " << std::get<1>(new_t2) << " " << std::get<2>(new_t2) << " " << std::get<3>(new_t2) << " " << std::get<4>(new_t2)  << endl;
                pq1.push(new_t2);
            }
            ...
            null_stream << x << " " << y << " " << z << " " << a << " " << b << " " << w << " " << w2 << " " << w3 << " " << p1 << " " <<  p2 << " " << p3 << endl;
